#!/usr/bin/python
# keeloq encryption cipher (160 rounds)
# 
...
PLAINTEXT = "01100010100101110000101011100011"
...
def decroundfunction(p,k,r):
  for i in range(0,r):
    newb = (int(k[31]) + int(p[31]) + int(p[15])  + core(int(p[30]),int(p[25]),int(p[19]),int(p[8]),int(p[0]))) % 2;
    shiftpd(p,newb);
    shiftkd(k);
